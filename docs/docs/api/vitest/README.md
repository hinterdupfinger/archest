**@archest/vitest**

***

# @archest/vitest

## Interfaces

- [ArchestMatchers](interfaces/ArchestMatchers.md)
- [ClassData](interfaces/ClassData.md)
- [ClassQueryOptions](interfaces/ClassQueryOptions.md)
- [FileData](interfaces/FileData.md)
- [FileQueryOptions](interfaces/FileQueryOptions.md)
- [FunctionData](interfaces/FunctionData.md)
- [FunctionQueryOptions](interfaces/FunctionQueryOptions.md)
- [ParseProjectOptions](interfaces/ParseProjectOptions.md)
- [ProjectData](interfaces/ProjectData.md)
- [PropertyData](interfaces/PropertyData.md)
- [PropertyQueryOptions](interfaces/PropertyQueryOptions.md)

## Functions

- [parseProject](functions/parseProject.md)
- [setupMatchers](functions/setupMatchers.md)
